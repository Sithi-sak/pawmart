...
<template>
  <div class="khqr-card">
    <svg
      class="khqr-card-frame"
      viewBox="0 0 442 622"
      fill="none"
      aria-hidden="true"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g filter="url(#khqrCardShadow)">
        <g clip-path="url(#khqrCardClip)">
      ...
      <defs>
        ...
        <clipPath id="khqrCardClip">
          <rect x="21" y="21" width="400" height="580" rx="24" fill="white" />
        </clipPath>
      </defs>
    </svg>

    <div class="khqr-card-details">
      <p class="khqr-card-company">{{ companyName }}</p>
      <p class="khqr-card-amount">
        {{ amount }}
        <span class="khqr-card-currency">USD</span>
      </p>
    </div>

    <div class="khqr-card-qr">
      <div v-if="qrSvg" class="khqr-card-qr-image" v-html="qrSvg" />
      <div v-else class="khqr-card-qr-loading">Generating code…</div>
    </div>
  </div>
</template>
...
.khqr-card-qr-image :deep(svg) {
  width: 100%;
  height: 100%;
}

.khqr-card-qr-loading {
  font-size: 0.75rem;
  color: #1a1a1a;
  opacity: 0.5;
  text-align: center;
